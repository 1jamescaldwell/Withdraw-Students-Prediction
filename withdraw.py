...
log_path = os.path.join(main_folder_path, r'logs\withdraw.log')
...
f = open(log_path, "a")
f.write("%s Starting data Load\n" % (dt))

# Import data
os.chdir(data_path)
withdraws = pd.read_csv('enrollment all years withdraw.csv')
all2324 = pd.read_csv('enrollment 23 and 24 all.csv')
all_data = pd.concat([withdraws, all2324])
gpa = pd.read_csv('GPA all.csv')
sat = pd.read_csv('SAT all.csv')
